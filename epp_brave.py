...
def scrape_data(url, num_items, directory):
    try:
        # Configurações do Brave (usando ChromeDriver)
        options = webdriver.ChromeOptions()
        options.headless = True  # não abre a janela
        options.binary_location = "/usr/bin/brave-browser"  # caminho do binário do Brave

        service = ChromeService(ChromeDriverManager(driver_version="139.0.7258.143").install())
